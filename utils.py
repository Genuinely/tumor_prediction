...
def normalize_data(opt, dtype, sequence):
    sequence.transpose_(0, 1)
    # print('first transpose:' + str(sequence.shape))
    sequence.transpose_(3, 4).transpose_(2, 3)
    # print('second transpose: ' + str(sequence.shape))
    return sequence_input(sequence, dtype)
...
# Not sure if the high part is necesarry for gray scale
...
def save_gif(filename, inputs, duration=1.0):
    images = []
...
def save_gif_with_text(filename, inputs, text, duration=1.0):
    images = []
    ...
    for i in range(bs):
        for t in range(T):
            for c in range(gt[t][i].shape[0]):
                try:
                    ssim[i, t] += ssim_metric(gt[t][i][c], pred[t][i][c], 1)
                except:
                    ssim[i, t] += 1
                psnr[i, t] += psnr_metric(gt[t][i][c], pred[t][i][c], 1)
